...
export default function getYupValidationSchema() {
  return Yup.object().shape({
    ...
    famousPeople: Yup.object({
      ...
      en: Yup.string().required('Знаменитости в кадре по-английски обязательны'),
    }),
    ...
    whatILikeHere: Yup.object({
      ru: Yup.string().required('Перечислите через запятую, что вам понравилось в фильме'),
      en: Yup.string().required(
        'Перечислите по-английски через запятую, что вам понравилось в фильме',
      ),
    }),
    kpID: Yup.string()
      .max(MAX_KP_ID_LENGTH, `Кинопоиск ID не может быть больше ${MAX_KP_ID_LENGTH} символов.`)
      .required('Кинопоиск ID обязателен'),
    imdbID: Yup.string()
      .max(IMDB_ID_LENGTH, `IMDb ID не может быть больше ${IMDB_ID_LENGTH} символов.`)
      .min(IMDB_ID_LENGTH, `IMDb ID не может быть меньше ${IMDB_ID_LENGTH} символов.`)
